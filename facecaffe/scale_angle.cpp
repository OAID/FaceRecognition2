...
static int get_probe_vec(float * landmark, int landmark_number, float * probe_vec, int probe_size, int desired_width, int desired_height)
{
   int i;
   ...
   for(i=0;i<5;i++)
   {
       mean_face_x[i]=(padding+mean_face_x[i])/(2*padding+1)*desired_width;
       mean_face_y[i]=(padding+mean_face_y[i])/(2*padding+1)*desired_height;
   } 
...
int cal_scale_and_angle(float * landmark, int landmark_number, int desired_width, int desired_height,float * scale, float * angle)
{
   float probe_vec[2];

   if(get_probe_vec(landmark,landmark_number,probe_vec,2,desired_width,desired_height)<0)
      return -1;
