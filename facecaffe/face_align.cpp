...
int get_aligned_face(const cv::Mat& img, float* landmark, int landmark_number, int desired_width, int desired_height,cv::Mat& out)
{
   ...
   if(cal_scale_and_angle(landmark,landmark_number,desired_width,desired_height,&scale,&angle)<0)
   {
        return -1;
   }


   to_center[0]=desired_width*0.5;
   to_center[1]=desired_height*0.4;
   ...
   cv::warpAffine(img,out,rot_mat,cv::Size(desired_width,desired_height));
